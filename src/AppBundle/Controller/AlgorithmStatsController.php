...
class AlgorithmStatsController extends Controller {
    ...
    public function getIndividualAlgorithmStatsAction(Request $request)
    {
        ...
        $algorithm_success_score = $request->get('algorithm_success_value');
        ...
        if(!$league || !$season || !$algorithm || $algorithm_success_score === '' || $minimum_history === '' || $include_x_league_form === '' || $minimum_league_form === ''){
            die;
        }
        ...
        foreach ($matches as $match)
        {
            ...
            if(array_key_exists('error', $algorithm_data) || $algorithm_data['score'] < $algorithm_success_score){
        ...
        return $this->render('algorithm-stats/partials/algorithmResults.html.twig', compact('league', 'season', 'algorithm_success_score', 'matches','prediction_count', 'correct'));
